...
export function ExplorersPanel() {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-ink-950/85 p-4 backdrop-blur">
      <div className="flex max-h-[85vh] w-full max-w-lg animate-pop-in flex-col rounded-3xl border border-gold-400/30 bg-ink-900 shadow-2xl">
        <header className="flex items-center justify-between border-b border-white/10 p-4">
          ...
          <button
            ...
            className="rounded-full bg-white/10 px-3 py-1 text-sm font-black text-white/75 transition hover:bg-clay-500/40"
          >
            ✕
          </button>
        </header>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          {/* you, always */}
          {player && (
            <div className="mb-4 flex items-center gap-3 rounded-2xl bg-gradient-to-r from-clay-500/25 to-gold-400/15 p-3 ring-1 ring-clay-400/40">
              <AvatarIcon id={player.avatarId} size={46} />
              <div className="min-w-0 flex-1">
                <div className="truncate font-extrabold text-white">
                  {player.nickname} <span className="text-xs font-bold text-clay-300">(you)</span>
                </div>
          ...
          {!supabaseEnabled && (
            <p className="rounded-xl bg-ink-800 p-3 text-xs font-semibold leading-5 text-white/55">
          ...
          {supabaseEnabled && (
            <>
              <h3 className="mb-2 text-xs font-extrabold uppercase tracking-widest text-gold-400">
                Photo leaderboard
              </h3>
                    ...
                    {rows.data.map((r, i) => (
                      <li
                        ...
                        className={`flex items-center gap-3 rounded-xl p-2 ${
                          r.id === player?.id ? 'bg-clay-500/20 ring-1 ring-clay-400/40' : 'bg-ink-800/70'
                        }`}
                      >
                        <span className="w-7 text-center text-lg">{MEDALS[i] ?? `${i + 1}.`}</span>
                        <AvatarIcon id={avatarById.has(r.avatar_id) ? r.avatar_id : 'sunny'} size={34} />
